...
- [ ] Добавить `@app.command(name="self-update") def self_update(...)` с опциональным флагом `--dry-run`
- [ ] Использовать `__version__` как текущую версию
- [ ] Использовать URL репозитория из `pyproject.toml` (`project.urls.Repository`) либо константу `Sergionius/cdt`
- [ ] При `--dry-run` только показать найденный latest release tag и команду обновления, не выполняя её
- [ ] При обычном запуске вызвать `run_self_update()` и завершиться с кодом subprocess
- [ ] Обработать случай, когда `pipx` не использовался для установки: вывести инструкцию вручную и завершиться с ошибкой
- [ ] Обновить `tests/test_cli.py`: проверить, что `cdt self-update --help` доступна, `--dry-run` возвращает 0 и содержит информацию о версии/команде
...
- [ ] Запустить линтер: `ruff check .`
- [ ] Проверить покрытие тестами: `pytest --cov=cdt --cov-report=term` (цель — 80%+)
