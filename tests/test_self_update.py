...
def test_latest_release_tag_parses_tag_name():
    response = FakeResponse(github_latest_release_json("v9.9.9"))
...
def test_update_command_for_pipx():
    cmd = _update_command("v9.9.9", "pipx", owner="Sergionius", repo="cdt")
    assert cmd == [
        "pipx",
        "install",
        "--force",
        "git+https://github.com/Sergionius/cdt.git@v9.9.9",
    ]


def test_update_command_for_pip():
    cmd = _update_command("v9.9.9", "pip", owner="Sergionius", repo="cdt")
    assert cmd[:3] == [sys.executable, "-m", "pip"]
    assert "--force-reinstall" in cmd
    assert "git+https://github.com/Sergionius/cdt.git@v9.9.9" in cmd


def test_update_command_unsupported_method_raises():
    with pytest.raises(SelfUpdateError, match="Unsupported"):
        _update_command("v9.9.9", "unknown", owner="Sergionius", repo="cdt")
...
def test_update_command_for_uv_requires_uv(monkeypatch):
    ...
    with pytest.raises(SelfUpdateError, match="uv is not available"):
        _update_command("v9.9.9", "uv", owner="Sergionius", repo="cdt")


def test_update_command_rejects_unsafe_tag():
    with pytest.raises(SelfUpdateError, match="Unsafe"):
        _update_command("v9.9.9 evil", "pipx", owner="Sergionius", repo="cdt")
...
def test_manual_update_command_quotes_url_and_python(monkeypatch):
    ...
    command = _manual_update_command("v9.9.9", owner="Sergionius", repo="cdt")
    assert "'/path with spaces/python3'" in command
    assert "pipx install --force git+https://github.com/Sergionius/cdt.git@v9.9.9" in command
    assert "git+https://github.com/Sergionius/cdt.git@v9.9.9" in command
...
def test_run_self_update_dry_run_prints_command(monkeypatch, capsys):
    ...
    assert "Latest release: v9.9.9" in captured.out
...
def test_run_self_update_uses_shell_safe_command_join(monkeypatch, capsys):
    ...
    assert "git+https://github.com/Sergionius/cdt.git@v9.9.9" in captured.out
...
def test_run_self_update_executes_command(monkeypatch, capsys):
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    assert recorded == [["pipx", "install", "--force", "git+https://github.com/Sergionius/cdt.git@v9.9.9"]]
...
def test_update_command_for_pip_includes_user_flag_when_requested():
    cmd = _update_command("v9.9.9", "pip", is_user=True, owner="Sergionius", repo="cdt")
    assert cmd[:3] == [sys.executable, "-m", "pip"]
    assert "--force-reinstall" in cmd
    assert "--user" in cmd
    assert "git+https://github.com/Sergionius/cdt.git@v9.9.9" in cmd
