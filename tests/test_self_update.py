...
def test_run_self_update_dry_run_prints_command(monkeypatch, capsys):
    monkeypatch.setattr(self_update, "__version__", "0.3.0")

    def fake_urlopen(url, **kwargs):
        return _FakeResponse(json.dumps({"tag_name": "v0.4.0"}).encode("utf-8"))
    ...
    assert exit_code == 0
    assert "Current version: 0.3.0" in captured.out
...
def test_run_self_update_already_up_to_date(monkeypatch, capsys):
    monkeypatch.setattr(self_update, "__version__", "0.4.0")
